...
/-!
This is a distance problem using the graph metric.

We can avoid actually doing the expansion by having nonuniform edge weights.
There are a number of ways to account for this.
Because it's a planar grid, we could do a trick where we just need to be
sure the edge weights sum to three when crossing a vertex associated to a column
(respectively row) that is being expanded.
```
      v
  |   |   |
--+-1-+-2-+--
  |   |   |
```
Since on a grid the graph metric is the L1 norm (Manhattan distance), this reduces to just
counting how many expanded rows and columns we cross and adding them to the distance.
-/
